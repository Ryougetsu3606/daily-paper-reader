---
title: Learning Counterfactual World Models for Embodied Reasoning under Partial Observability
title_zh: 在部分可观测条件下学习用于具身推理的反事实世界模型
authors: "Todd Y. Zhou, Daniel Zhang"
date: 2026-09-05
pdf: "https://arxiv.org/pdf/2609.05834v1"
tags: ["query:ego-embodied"]
score: 8.0
evidence: 面向具身推理的反事实世界模型与可行动表征
tldr: "世界模型常因只优化感知相似度而出现“反事实坍缩”：预测看似合理的未来，却无法区分行为后果不同的干预。为此提出反事实潜世界模型 CLWM，结合循环信念编码器、动作条件潜动态与对比式反事实目标，在观测相似时仍分离不同干预的未来。在遮挡推物、别名导航和长时程操作中，规划成功率由65.1%升至74.6%、67.3%升至78.9%，并降低利用性规划失败。论文还给出反事实可分性指标，可审计编码器是否值得规划器信任。"
source: arxiv
selection_source: carryover_cache
figures_json: "[{\"url\": \"assets/figures/arxiv/2609.05834v1/fig-001.webp\", \"caption\": \"\", \"page\": 0, \"index\": 1, \"width\": 1017, \"height\": 498, \"label\": \"Figure\"}, {\"url\": \"assets/figures/arxiv/2609.05834v1/fig-002.webp\", \"caption\": \"\", \"page\": 0, \"index\": 2, \"width\": 1062, \"height\": 496, \"label\": \"Figure\"}]"
tables_json: "[{\"url\": \"assets/tables/arxiv/2609.05834v1/table-001.webp\", \"caption\": \"\", \"page\": 0, \"index\": 1, \"width\": 1212, \"height\": 387, \"label\": \"Table\"}, {\"url\": \"assets/tables/arxiv/2609.05834v1/table-002.webp\", \"caption\": \"\", \"page\": 0, \"index\": 2, \"width\": 1410, \"height\": 374, \"label\": \"Table\"}, {\"url\": \"assets/tables/arxiv/2609.05834v1/table-003.webp\", \"caption\": \"\", \"page\": 0, \"index\": 3, \"width\": 1445, \"height\": 460, \"label\": \"Table\"}, {\"url\": \"assets/tables/arxiv/2609.05834v1/table-004.webp\", \"caption\": \"\", \"page\": 0, \"index\": 4, \"width\": 1441, \"height\": 852, \"label\": \"Table\"}]"
motivation: 现有大规模预训练编码器多按感知相似度优化，导致世界模型预测视觉合理却无法区分后果不同的干预，即反事实坍缩。
method: 提出 CLWM，包含循环信念状态编码器、动作条件潜动态，以及对比式反事实目标，用硬反事实负样本分离外观相似但干预不同的未来。
result: "遮挡推物、别名导航和长时程操作中，成功率从65.1%升至74.6%、67.3%升至78.9%，利用性失败从18.4%降至9.7%。"
conclusion: 反事实可分性指标与五类基线规划成功率高度相关（r≥0.94），可基于干预-结果标签审计任意编码器；目前仅验证从零训练的世界模型。
---

## 摘要
世界模型有望为具身智能提供一条通用路径：一次性学习预测动力学，然后用其进行推理、规划和行动。如今，这类模型底层的表示越来越多地在大规模视频、交互和多模态语料库上预训练，这引出了一个仅靠预测质量无法回答的问题：学到的表示何时才真正可付诸行动？我们识别出一种称为反事实坍缩的失败模式：模型能预测视觉上看似合理的未来，却无法区分具有不同行为后果的干预。只要表示是面向感知相似性而非干预结构进行优化，就会出现这种情况，而这恰恰是大多数大规模预训练编码器所采用的学习目标。我们提出反事实潜在世界模型（CLWM），它结合了循环信念状态编码器、动作条件潜在动力学，以及一个对比式反事实目标，从而将不同干预所诱导的未来区分开，即使它们的观测看起来相似。在遮挡操作、混叠导航和长时程操作中，CLWM 相比最强基线提升了规划成功率（Occluded Push 上从 65.1% 提升至 74.6%，Aliased Maze 上从 67.3% 提升至 78.9%），并减少了利用性规划失败（Deferred Kitchen 上从 18.4% 降至 9.7%）；消融实验将增益归因于困难反事实负样本，尤其是感知混叠负样本。最后，我们的反事实可分离性指标在五类基线模型上追踪规划成功率（r ≥ 0.94），并且与表示无关：给定干预—结果标签，它可以在规划器信任任何编码器之前对其进行审计，无论该编码器是预训练的还是从头训练的。我们尚未在大规模预训练编码器上测量该指标。本文针对从头训练的世界模型，建立了该指标及其与规划成功率之间的关系。

## Abstract
World models promise a general route to embodied intelligence: learn predictive dynamics once, then reason, plan, and act with them. Increasingly, the representations beneath such models are pretrained on large-scale video, interaction, and multimodal corpora, which raises a question prediction quality alone cannot answer: when is a learned representation actually actionable? We identify a failure mode we call counterfactual collapse: a model predicts visually plausible futures while failing to distinguish interventions with different behavioral consequences. This arises whenever a representation is optimized for perceptual similarity rather than intervention structure, which is precisely the objective under which most large-scale pretrained encoders are learned. We introduce Counterfactual Latent World Models (CLWM), which combine a recurrent belief-state encoder, action-conditioned latent dynamics, and a contrastive counterfactual objective that separates futures induced by distinct interventions even when their observations look alike. Across occluded manipulation, aliased navigation, and long-horizon manipulation, CLWM improves planning success over the strongest baseline (65.1% $\to$ 74.6% on Occluded Push and 67.3% $\to$ 78.9% on Aliased Maze) and reduces exploitative planning failures (18.4% $\to$ 9.7% on Deferred Kitchen), with ablations attributing the gains to hard counterfactual negatives, especially perceptual-alias negatives. Finally, our counterfactual separability metric, which tracks planning success across the five baseline model classes ($r \ge 0.94$), is representation-agnostic: given intervention-outcome labels, it can audit any encoder, pretrained or trained from scratch, before a planner trusts it. We do not yet measure it on large-scale pretrained encoders. Here we establish the metric and its relationship to planning success for world models trained from scratch.